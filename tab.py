...
suma_por_cliente["% de costo"] = (
    suma_por_cliente["MONTO ADEUDADO"] / suma_por_cliente["MONTO ADEUDADO"].sum() * 100
)
...
def replace_deudores(x):
    top_10 = (
        topdeudores.groupby("NOMBRE")["ADEUDO POR CLIENTE"].sum().nlargest(10).index
    )
    if x not in top_10:
...
if agree:
    ...
    color_scale = alt.Scale(
        domain=list(datas["MAG_PLAZO"].unique()), range=custom_colors
    )
    ...
    chart = (
        ...
        .encode(
            ...
            y="MONTO ADEUDADO",
            color=alt.Color(
                "MAG_PLAZO", scale=color_scale
            ),  # Utiliza la escala de colores personalizada
            tooltip=["FECHA_VENCIMIENTO:T", "MONTO ADEUDADO", "NOMBRE"],
        )
        .transform_filter(click)
        .properties(title="Monto adeudado por fecha y plazo")
    )
    ...
    monto_graf = (
        alt.Chart(topdeudores)
        .mark_bar()
        .encode(
            x=alt.X(
                "NOMBRE",
                title="Nombre del cliente",
                axis=alt.Axis(titleColor="black", labelAngle=90),
            ...
            tooltip=['NOMBRE',"MONTO ADEUDADO", "FECHA_VENCIMIENTO"],
        )
        .properties(title="Monto Adeudado por Cliente", width = 800, height = 500)
    )
    monto_graf
...
productos["MARGEN %"] = (
    productos["MARGEN_UNITARIO_CALCULADO"] / productos["PRECIO_UNITARIO"] * 100
)
suma_por_cliente = productos.groupby("NOMBRE_CLIENTE")[
    "MARGEN_UNITARIO_CALCULADO"
].sum()
productos["MARGEN CLIENTE"] = productos["NOMBRE_CLIENTE"].map(suma_por_cliente)
productos["% DEL MARGEN CLIENTE"] = (
    productos["MARGEN CLIENTE"] / productos["MARGEN CLIENTE"].sum()
)
venta_por_producto = productos.groupby("CVE_ART")["VENTA TOTAL"].sum()
venta_por_producto = pd.DataFrame(venta_por_producto)
venta_por_producto["% de venta"] = (
    venta_por_producto["VENTA TOTAL"] / venta_por_producto["VENTA TOTAL"].sum() * 100
)
...
costo_por_producto["% de costo"] = (
    costo_por_producto["COSTO TOTAL"] / costo_por_producto["COSTO TOTAL"].sum() * 100
)
...
margen_por_producto["% de margen"] = (
    margen_por_producto["MARGEN TOTAL"]
    / margen_por_producto["MARGEN TOTAL"].sum()
    * 100
)
...
def replace_clientes(x):
    top_10 = (
        topclientes.groupby("NOMBRE_CLIENTE")["MARGEN CLIENTE"].sum().nlargest(10).index
    )
    if x not in top_10:
...
def replace_productos(x):
    top_10 = (
        topproductos.groupby("CVE_ART")["MARGEN POR PRODUCTO"].sum().nlargest(10).index
    )
    if x not in top_10:
        return "OTROS"  # Valor de reemplazo
    else:
...
if agree2:
    ...
    precio = (
        ...
        .properties(title="Productos Vendidos", width=600)
    ...
    barras = (
        ...
        .properties(title="Aportación a utilidad bruta por vendedor", width=300)
    )
    ...
    barras2 = (
        ...
        .transform_filter(interval)
        .properties(title="Aportación a utilidad bruta por producto", width=300)
    )
...
if agree5:
    ...
    legend = (
        alt.Chart(
            pd.DataFrame(
                {"label": ["Proveedores", "Saldos"], "color": ["red", "green"]}
            )
        )
        .mark_text()
        .encode(
...
egreso_por_proveedor["% de egreso"] = (
    egreso_por_proveedor["TOTAL_SAT"] / egreso_por_proveedor["TOTAL_SAT"].sum() * 100
)
...
egreso_por_clase["% de egreso"] = (
    egreso_por_clase["TOTAL_SAT"] / egreso_por_clase["TOTAL_SAT"].sum() * 100
)
...
def replace_proveedores(x):
    top_10 = (
        topproveedores.groupby("PROVEEDOR")["EGRESO POR PROVEEDOR"]
        .sum()
        .nlargest(20)
        .index
    )
...
if agree3:
    gastos_año = gastos[gastos['AÑO'] == año_seleccionado]
    gastos_sum = gastos_año.groupby(['CLASIFICACION', 'TIPO_GASTO'])['TOTAL_SAT'].sum().reset_index()
    ...
    graf = px.sunburst(
        ...
        color="TOTAL_SAT",
        values="TOTAL_SAT",
        path=["CLASIFICACION", "TIPO_GASTO"],
        hover_name="TIPO_GASTO",
        title="Gastos y costos",
        color_continuous_scale=color_scale,
    )
    
    topp_año = topproveedores[topproveedores['AÑO'] == año_seleccionado]

    graf2 = px.treemap(
        topp_año,
        values="TOTAL_SAT",
        path=["AÑO", "CLASIFICACION", "PROVEEDOR"],
        hover_name="PROVEEDOR",
        color_discrete_sequence=['#bd1422'],
    )
...
# Convertir las columnas 'AÑO' y 'MES' a enteros
    ...
    fact_año = fact[fact['AÑO'] == año_seleccionado]

    # Crear el gráfico de barras
    a = alt.Chart(fact_año).mark_bar().encode(
    ...
    ## Gráfica 2
    # Crear el gráfico de barras
    b = (alt.Chart(fact_año)
        .mark_bar()
        .encode(
            x=alt.X("MES:O", title="MES"),
            y=alt.Y("sum(CAN_TOT)", title="Monto acumulado de facturas"),
            color=alt.Color(
                "MES:O", scale=alt.Scale(scheme="tableau20"), title="Mes"
            ),
            tooltip=["sum(TOTAL)", "count(CVE_DOC)"],
        )
        .properties(title="Total de facturas por mes", height=400, width=800)
    )
    ...
    tclif_año = tclif[tclif['AÑO'] == año_seleccionado]
    ...
    c = (
        ...
        .properties(title="Facturación por cliente", width=800)
    )
    ...
    d = (
        ...
        .properties(title="Facturación por Vendedor", width=800)
    )
...
suma_por_cliente = pd.DataFrame(suma_por_cliente)
suma_por_cliente["% de fact"] = (
    suma_por_cliente["CAN_TOT"] / suma_por_cliente["CAN_TOT"].sum() * 100
)
psuma_por_cliente = pd.Series(suma_por_cliente["% de fact"])
...
suma_por_vendedor = pd.DataFrame(suma_por_vendedor)
suma_por_vendedor["% de fact"] = (
    suma_por_vendedor["CAN_TOT"] / suma_por_vendedor["CAN_TOT"].sum() * 100
)
psuma_por_vendedor = pd.Series(suma_por_vendedor["% de fact"])
...
if agree5:
    ...
    notas_año = notas[notas['AÑO'] == año_seleccionado]

    # Crear el gráfico de barras
    g1 = alt.Chart(notas_año).mark_bar().encode(
            x=alt.X("FECHA_DOC", title="Fecha"),
            y=alt.Y("CAN_TOT", title="Monto acumulado de notas de credito"),
            color=alt.Color(
                "MAG_NOTAS", scale=alt.Scale(scheme="reds"), title="Magnitud de Notas"
            ),
            tooltip="CAN_TOT",
        ).properties(title="Notas de crédito", height=400, width=800)
    
    ## Gráfica 2
    g2 = alt.Chart(notas_año).mark_bar().encode(
            x=alt.X(
                "MES:O",
                title="MES",
            ),
            y=alt.Y("sum(CAN_TOT)", title="Monto acumulado de notas de crédito"),
            color=alt.Color(
                "MES:O", scale=alt.Scale(scheme="tableau20"), title="Mes"
            ),
            tooltip=["sum(CAN_TOT)", "count(CAN_TOT)"],
        ).properties(
            title="Total de notas de crédito por mes", height=400, width=800
            )
...
if agree6:
    ...
    flujo['INGRESOS'] = flujo['CAN_TOT_x'] + flujo['CAN_TOT_y']
    flujo['EGRESOS'] = flujo['TOTAL_SAT']

    flujo['FLUJO'] = flujo['INGRESOS'] - flujo['EGRESOS']
    flujo = flujo[['INGRESOS','EGRESOS','FLUJO']]
    ...
    df_melted["Tipo"] = pd.Categorical(
        df_melted["Tipo"], categories=orden_tipos, ordered=True
    )

# Lista desplegable en Streamlit
    
    @st.cache_data
    def filtrar_por_año(data, año):
        return data[data['AÑO'] == año]

    df_filtrado = filtrar_por_año(df_melted, año_seleccionado)
    ...
    main_chart = (
        ...
        .properties(
            width=200, height=400, title="Desglose de Ingresos, Egresos y Flujo por Mes"
        )
    )
